...
double Dalal = 2.4e-5*exp(-18040.0/T) ; //0.001 ;        // The next four lines are the components of the Onsager mobility matrix
double Dvv = 1.0e-5*exp(-17460.0/T) ; //0.001 ;
double Dnorm = Dalal ;
double L_norm = (Dnorm*Vm)/(R*T*lc*lc) ;
double T_trans = 1268.0 ;
double fp = 2.0e-03 ; 
double L_orig = fp/(T_trans-T) ; //v_alpha*T_trans/((T_trans - T)*heat*5e-07) ;
...
//#include "/home/baskaa2/mmsp-arun/fftw++-2.05/fftw++.h"
