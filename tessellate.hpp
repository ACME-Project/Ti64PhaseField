...
void exact_voronoi(MMSP::grid<dim, sparse<T> >& grid, const std::vector<std::vector<MMSP::vector<int> > >& seeds) {
  ...
  unsigned long tstart=time(NULL);
	...
	for (int n=0; n<nodes(grid); ++n) {
		//if (id==0) print_progress(n,nodes(grid),1);
...
template<int dim, typename T>
void seeds_from_gradient(const MMSP::grid<dim, sparse<T> >& grid, const int& nseeds, const int& num_bins, const int& bias, std::vector<MMSP::vector<int> >& local_seeds) {
	int id=0;
	#ifdef MPI_VERSION
		id=MPI::COMM_WORLD.Get_rank();
	#endif
	...
	unsigned long int pseudorand_seed = time(NULL);
	if (id == 0) 
		std::cout << "Master seed is " << std::setw(10) << std::right << pseudorand_seed << ". <--- Record this value!" << std::endl;
	#ifdef MPI_VERSION
		pseudorand_seed = pseudorand_seed / (id + 1);
	#endif
	MTRand pseudorand_number( pseudorand_seed );
	...
	// deal out the number of seeds from bias to each bin
	for (int bin_num = 0; bin_num < num_bins; bin_num++){
		int grains_to_seed = bin_num * bias;
		for (int i=0; i<grains_to_seed; ++i) {
			for (int d=0; d<dim; ++d){
				// global maxima and minima dimensional bounds are defined differently
				// for parallel applications (g0/g1) than serial (x0/x1)
				#ifdef MPI_VERSION
					int inc = (g1(grid,d)-g0(grid,d))/num_bins;
					int lower_bound = g0(grid,d) + (d==0)*(bin_num * inc);
					int upper_bound = g1(grid,d) - (d==0)*(g1(grid,d) - ((bin_num+1)* inc + g0(grid,d)));
					s[d] = lower_bound+pseudorand_number.randInt(upper_bound-lower_bound);
				#else
					int inc = (x1(grid,d)-x0(grid,d))/num_bins;
					int lower_bound = x0(grid,d) + (d==0)*(bin_num * inc);
					int upper_bound = x1(grid,d) - (d==0)*(x1(grid,d) - ((bin_num+1)* inc + x0(grid,d)));
					s[d] = lower_bound+pseudorand_number.randInt(upper_bound - lower_bound);
				#endif
			}
			local_seeds.push_back(s);
	...
	int remainder = nseeds - local_seeds.size();
	// "deal out" seeds to each bin until the "deck" of seeds is exhausted
	for (int i=0; i<remainder; ++i) {
		int bin_num = i%num_bins;
		for (int d=0; d<dim; ++d){
			// global maxima and minima dimensional bounds are defined differently
			// for parallel applications (g0/g1) than serial (x0/x1)
			#ifdef MPI_VERSION
...
template<int dim, typename T>
void tessellate(MMSP::grid<dim, sparse<T> >& grid, const int& nseeds, const int& num_bins, const int& bias) {
  int np=1;
	#ifdef MPI_VERSION
  np=MPI::COMM_WORLD.Get_size();
	#endif
  ...
  while (int(seeds.size()) <= np) seeds.push_back(local_seeds); // avoid a segfault
	...
	seeds_from_gradient(grid, nseeds, num_bins, bias, local_seeds);
	tessellate(local_seeds, seeds, grid);
