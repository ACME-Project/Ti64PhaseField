...
int main(int argc, char* argv[])
{
	...
	if (std::string(argv[1]) == "--help") {
		std::cout << argv[0] << ": convert MMSP grid data to comma-delimited ASCII (CSV) format.\n";
	...
	if (argc==2) {
		...
		if (datname.find_first_of("0123456789",extpos) != std::string::npos)
			...
			csvname << datname.substr(0, extpos) << ".csv";
	} else
		csvname << argv[2];

	// Open output file
	std::ofstream csvfil(csvname.str().c_str());
